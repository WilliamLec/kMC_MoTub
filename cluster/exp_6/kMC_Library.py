...
def reac(i, j, lattice, occ, ersl, m, is_scan=False, idx=0):
    """
...
def initialize_event_lists(lat, Nr, m): 
    ...
    for i in range(n):
        for j in range(m):
            occ, ersl = reac(i, j, lat, occ, ersl, m, False, 0)
    return (occ, ersl)

def scan(lat, loc, ersl, occ, m): 
    ...
    for index in range(nbr_neighbors): 
        i = int(conv[index,0])
        j = int(conv[index,1])
        occ_neighborhood, ersl_neighborhood = reac(i, j, lat, occ_neighborhood, ersl_neighborhood, m, True, index)
    occ += occ_neighborhood
...
def update_lattice_configuration(lat, occ, ersl, reac, loc, m):
    ...
    """
    occ, ersl = scan(lat, loc, ersl, occ, m)           
    return lat, occ, ersl
