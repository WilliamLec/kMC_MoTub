...
def reaction_test(i, j, lattice, occ, ersl, TABLE_NO_DEFECT, TABLE_DEFECT, D, is_scan, idx):
    """
...
def initialize_event_lists(lat, Nr, TABLE_NO_DEFECT, TABLE_DEFECT, D): 
    ...
    for i in range(n):
        for j in range(m):
            occ, ersl = reaction_test(i, j, lat, occ, ersl,
                                      TABLE_NO_DEFECT, TABLE_DEFECT, D,
                                      False, 0)
    return (occ, ersl)

def scan(lat, loc, ersl, occ, DIRECTIONALITY, TABLE_NO_DEFECT, TABLE_DEFECT): 
    ...
    for index in range(nbr_neighbors): 
        i = int(conv[index,0])
        j = int(conv[index,1])
        occ_neighborhood, ersl_neighborhood = reaction_test(i, j, lat, occ_neighborhood, ersl_neighborhood,
                                                            TABLE_NO_DEFECT, TABLE_DEFECT, D,
                                                            True, index)
    occ += occ_neighborhood
...
def update_lattice_configuration(lat, occ, ersl, reac, loc, DIRECTIONALITY, TABLE_DEFECT, TABLE_NO_DEFECT):
    ...
    """
    occ, ersl = scan(lat, loc, ersl, occ, DIRECTIONALITY,
                     TABLE_NO_DEFECT, TABLE_DEFECT)    
    return lat, occ, ersl
